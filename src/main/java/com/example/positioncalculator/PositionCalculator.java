...
public final class PositionCalculator {
    ...
    public static BigDecimal calculatePosition(final BigDecimal capital,
        ...
        BigDecimal answer = riskSumPerTrade(capital, riskInPercent).divide(riskSumPerUnit(price, stopLossPrice, direction), 0, BigDecimal.ROUND_DOWN);
        if (capital.compareTo(answer.multiply(price)) != 1){
