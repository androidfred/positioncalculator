...
public abstract class Position {
    ...
    public final BigDecimal getTotalTolerableRiskPerTrade() {
        return capital.getCapital().multiply(tolerableRiskInPercentOfCapital.getTolerableRiskInPercentOfCapital().divide(new BigDecimal(100)));
    }
    ...
    public final BigDecimal getUnitsToBuy() {
        ...
        if (capital.getCapital().compareTo(result.multiply(pricePerUnit.getPricePerUnit())) != 1) {
    ...
    public final BigDecimal getTotal() {
        return getUnitsToBuy().multiply(pricePerUnit.getPricePerUnit());
    }
